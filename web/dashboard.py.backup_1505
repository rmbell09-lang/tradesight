...
def get_strategy_lab_stats():
    ...
    try:
        ...
        return {
            'strategies_tested': len(results['participants']),
            'winner': results['winner']['name'] if results['winner'] else 'None',
            'winner_score': results['winner']['avg_score'] if results['winner'] else 0,
